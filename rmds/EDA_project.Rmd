...
El presente rmarkdown tiene como objetivo elaborar un análisis exploratorio para la busqueda de patrones de cumplimiento en las variables a emplear en el modelo de regresión lógistica, así como también proponer limites y filtros a cada variable continua en caso de existir valores presuntamente atipicos. Estas propuestas de filtros se evaluaran y se propondran en el análisis univariado y serán valorados al final del rmarkdown para generar 3 muestras (muestra completa, submuestra 1 y submuestra 2) con las que en cada una se ajustaran modelos logit, para finalmente comparar los resultados de cada modelo y elegir el de mejor performance contra el estadístico de prueba (% de aciertos).
...
```{r, include=FALSE}
gather(select(Train_copy_continuas, -c(Maximum.Open.Credit, Current.Loan.Amount, Annual.Income, Current.Credit.Balance, Monthly.Debt, Credit.Score, Bankruptcies, Tax.Liens)), key = "variable", value = "valor", -Loan.Status)
```
...
Comportamiento del Annual.Income vs Monthly.Debt (color by Current.Loan.Amount) con faceteado por Term

```{r}
ggplot(Train_copy, aes(Annual.Income, Monthly.Debt, color=Current.Loan.Amount))+
  geom_point()+
  facet_wrap(~Term, ncol = 2)+
  theme_bw()
```

Comportamiento del Annual.Income vs Monthly.Debt (color by Current.Loan.Amount) con faceteado por LoanStatus

```{r}
ggplot(Train_copy, aes(Annual.Income, Monthly.Debt, color=Loan.Status))+
  geom_point()+
  facet_wrap(~Term, ncol = 2)+
  theme_bw()
```
...
ggplot(Train_copy, aes(Annual.Income, Monthly.Debt, color=Loan.Status))+
  geom_point()+
  facet_wrap(~Years.in.current.job, ncol =4)+
  theme_bw()
```
...
ggplot(Train_copy, aes(Annual.Income, Monthly.Debt, color=Loan.Status))+
  geom_point()+
  facet_wrap(~Home.Ownership, ncol =4)+
  theme_bw()
```
