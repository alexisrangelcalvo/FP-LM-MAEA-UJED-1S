...
MSE_kf_1_1$delta
```
Salida de consola y código no.9 (MSPR por K-Pliegues del modelo no. 1.1)-
...
MSE_kf_1_2$delta
```
Salida de consola y código no. 10 (MSPR por K-Pliegues del modelo no. 1.2)-
...
exac2 = ggplot(df_especificidad, aes(x=Corte_1, y=Exactitud_2))+
  ...
  scale_y_continuous(breaks=c(0.2, 0.4, 0.5, 0.6, 0.7, 0.8), labels = c("20%", "40%", "50%", "60%", "70%", "80%"),
                     name = "Exactitud")+
  scale_x_continuous("Punto de corte")+
  ggtitle("Punto de corte ideal para máximizar la exactitud de la clásificación (modelo 1.2)")+
  theme_bw()
exac2
```
...
```{r}
cfmx_modelo1 = cfmx1
save(modelo_1_1, cfmx_modelo1, file = "../models/Modelo_1.RData")
```
