...
```{r}
suppressWarnings(MSE_kf_2_1 <- cv.glm(data = Test_intern_1,
               glmfit = modelo_2_1, 
               K = 50))
MSE_kf_2_1$delta
```
...
```{r}
suppressWarnings(MSE_kf_2_2 <- cv.glm(data = Test_intern_1,
               glmfit = modelo_2_2, 
               K = 50))
MSE_kf_2_2$delta
```
...
df <- data.frame(valor = c(bootstrap_vector_modelo_2_1, bootstrap_vector_modelo_2_2),
                 grupo = factor(rep(c("Modelo 1.2", "Modelo 1.3"), each = length(bootstrap_vector_modelo_2_1))))
...
```{r}
cfmx_modelo2 = cfmx2
save(modelo_2_2, cfmx_modelo2, file = "../models/Modelo_2.RData")
```
