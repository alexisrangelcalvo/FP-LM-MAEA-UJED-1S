...
# glmulti.logistic.out <- glmulti(Loan.Status ~ Term + Home.Ownership + Purpose + 
    # Current.Loan.Amount + Credit.Score + Annual.Income + Monthly.Debt + 
    # Years.of.Credit.History, data = Train_intern_1,, data = Train_intern_1,
...
```
```{r}
Prediccion_1<-as.factor(ifelse(prediccion_1<=Corte_1[corte_optimo_1],yes = 1, no = 0))
...
```{r}
Prediccion_2<-as.factor(ifelse(prediccion_2<=Corte_1[corte_optimo_1],yes = 1, no = 0))
confusionMatrix(data = as.factor(Test_intern_1$Loan.Status), 
                             Prediccion_2)
...
- Si la variable `Purpose` adopta el valor `wedding`, este evento estara asociado con un aumento del 25% en las probabilidades del pago al corriente (`Fully Paid`).
...
```{r}
save(modelo_1_3, cfmx2, file = "../models/Modelo_2.RData")
```
