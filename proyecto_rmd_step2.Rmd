...
```{r}
Train_copy_continuas = Train_copy %>% dplyr::select(Current.Loan.Amount, Credit.Score, Annual.Income, Monthly.Debt, Years.of.Credit.History, Months.since.last.delinquent, Number.of.Open.Accounts, Current.Credit.Balance, Maximum.Open.Credit, Bankruptcies, Tax.Liens)
...
```{r}
train_long_form = gather(dplyr::select(Train_copy_continuas, -c(Maximum.Open.Credit, Current.Loan.Amount, Annual.Income, Current.Credit.Balance, Monthly.Debt, Credit.Score, Bankruptcies, Tax.Liens)), key = "variable", value = "valor")
...
```{r}
train_selected = dplyr::select(Train_copy_continuas, Maximum.Open.Credit, Current.Loan.Amount)
...
```{r}
train_selected = dplyr::select(Train_copy_continuas, Current.Loan.Amount)
train_long_form1 = gather(train_selected, key = "variable", value = "valor")
...
```{r}
train_selected = dplyr::select(Train_copy_continuas, Annual.Income)
...
```{r}
train_selected = dplyr::select(Train_copy_continuas, Current.Credit.Balance)
...
```{r}
train_selected = dplyr::select(Train_copy_continuas, Monthly.Debt)
...
```{r}
train_selected = dplyr::select(Train_copy_continuas, Credit.Score)
...
```{r}
train_selected = Train_copy_continuas %>% filter(Credit.Score <= 738) %>% dplyr::select(Credit.Score)
...
```{r}
train_selected = Train_copy_continuas %>% filter(Credit.Score > 753) %>% dplyr::select(Credit.Score)
...
train_selected = Train_copy_continuas %>% dplyr::select(Credit.Score)
...
- Bankruptcies
```{r}
train_selected = dplyr::select(Train_copy_continuas, Tax.Liens, Bankruptcies)
train_long_form6 = gather(train_selected, key = "variable", value = "valor")
...
```
- probar graficas de multiples diferencias para evluar caracteristicas entre gupos
...
Train_copy_1_continues = Train_copy_1 %>% dplyr::select(Current.Loan.Amount, Credit.Score, Annual.Income, Monthly.Debt, Years.of.Credit.History, Months.since.last.delinquent, Number.of.Open.Accounts, Current.Credit.Balance, Maximum.Open.Credit, Bankruptcies, Tax.Liens)

chart.Correlation(dplyr::select(Train_copy_1_continues, -c(Tax.Liens)), histogram=TRUE, pch=19, main = "Matriz de correlaciones con histógrama")
```
...
```{r}
Train_copy_1_model = dplyr::select(Train_copy_1, -c(ID))
...
##### Cumplimiento de los supuestos del modelo de regresion logistica

- Multicolinealidad (Factor de inflación de la varianza o VIF)
...
vif_modelo <- vif(modelo_multiple_1)
...
##### Ajuste del modelo de regresión logística (modelo 2 con seleccion de variables)
...
De acuerdo con los estadísticos recomendados para el conocimiento del modelo, ¿cuántas variables deberiamos de tener en el el modelo y qué variables?

```{r}
reg_summary = summary(Subs)
...
```
Es decir que de cada variable nominal u ordinal, ¿no todas se estarían seleccionando?

Selección y ajuste de modelo por el estadístico cp de mallow
...
######   Identificando intervalos de confianza  y  coeficientes para el modelo no. 2
```{r}
...
######  Modelo optimizado con el punto máximo de la curva de exactitud 


- punto de corte ideal para maximizar el accuracy
...
```
- ubicando tal punto de corte óptimo para maximizar el accuracy
...
for (i in 1:length(Corte_2)) {
  ...
  Exactitud_2[i] <- tryCatch(
    {
      confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), Prediccion)$byClass[1]
    },
    error = function(e) {
      0 # O cualquier valor predeterminado
    }
...
plot(x=Corte_2, y = Exactitud_2, type = "l", main ="Punto de corte óptimo para la sensibilidad", xlab="Corte", ylab="Exactitud")
```
- ubicando tal punto de corte óptimo para maximizar la sensibilidad
...
for (i in 1:length(Corte_2)) {
  ...
  Exactitud_2[i] <- tryCatch(
    {
      confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), Prediccion)$byClass[2]
    },
    error = function(e) {
      0 # O cualquier valor predeterminado
    }
...
plot(x=Corte_2, y = Exactitud_2, type = "l", main ="Punto de corte óptimo para la especificidad", xlab="Corte", ylab="Exactitud")
```
- ubicando tal punto de corte óptimo para maximizar el especificidad
...
###### Predicción y matriz de confusión para el modelo no. 2 con punto de corte optimo empleado
...
```
Estudiando el objeto matriz de confusión

```{r}
names(confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), 
                             Prediccion_2_1))

confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), 
                             Prediccion_2_1)$byClass
```
...
Prediccion_2_1<-as.factor(ifelse(prediccion_2<=0.44,yes = 1, no = 0))
confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), 
                             Prediccion_2_1)
```
...
##### Ajuste del modelo de regresión logística (modelo 3 con seleccion de variables por PRESS)

- Se decide ajustar un 3er modelo pero en esta ocasión ajustandolo por el estadístico PRESS. Lo anterior con la finalidad de comparar los tres modelos ajustados y quedarnos con el de mejor desempeño para la predicción y clásifiación de nuevas obervaciones.

Empleando el PRESS (predicted residual error sum of squares) como selector
```{r}

# Crear una matriz de diseño con todas las variables independientes
x <- model.matrix(modelo_multiple_1)[,-1]
y <- Train_copy_1_model$Loan.Status

# Buscar el mejor subconjunto de predictores utilizando PRESS
press.regsubsets <- regsubsets(x, y, nvmax = ncol(x), method = "exhaustive", criterion = "press")
summary(press.regsubsets)
```

Los resultados arrojan que entre menos variables predictoras menor será el PRESS 

```{r}
nvmax <- dim(x)[2]
plot(1:nvmax, press.regsubsets$press, type = "b", xlab = "Number of Variables", ylab = "PRESS", pch = 19)

min.press <- which.min(press.regsubsets$press)
abline(v = min.press, lty = 2)
```
```{r}
coef(press.regsubsets, id = 3)
```
De esta forma, procedemos a ajustar un modelo con solo 3 variables predictoras (Term, Credit.Score, Annual.Income )

```{r}
modelo_multiple_3 = glm(data = Train_copy_1_model, 
              formula = Loan.Status~Term + Credit.Score + Annual.Income, family = "binomial")
...
```{r}
modelo_multiple_3$aic
```
La diferencia entre el AIC del modelo 2 y el 3 es 163.02 (26657.84-26494.82)


######   Identificando intervalos de confianza  y  coeficientes para el modelo no. 3
```{r}
confint(object = modelo_multiple_3, level = 0.95)
```

- Ecuación para el modelo no. 3


```{r}
# Obtener el vector de coeficientes del modelo
coeficientes <- coef(modelo_multiple_3)
...
cat("La ecuación del modelo es:\n", cadena_ecuacion, "\n")
```
la beta del Anual.income es cero?
...
###### Identificando los coeficientes para el modelo no. 3
 
```{r}
nombres_vars = names(coef(modelo_multiple_3))
resumen = summary(modelo_multiple_3)
...
###### ODDS Ratios para el modelo no. 3
- Identificando los odds ratios 
$e^{B_{k}}$
...
###### Predicción y Matriz de confusión para el modelo no. 3

- Predicción

Estableciendo una predicción con punto de corte arbitrario de 0.85
```{r}
prediccion_3 = predict(object = modelo_multiple_3,
                    newdata = Train_copy_1_model, type = "response")


Prediccion_3<-as.factor(ifelse(prediccion_3<=0.85,yes = 1, no = 0))
...
- Matriz de confusión para el modelo 1 plus con punto de corte de 0.85 (arbitrario)
```{r}
confusionMatrix(reference = as.factor(Train_copy_1_model$Loan.Status), Prediccion_3)
...
```{r}
Exactitud_3 <- vector()
Corte_3 <- seq(0.05, 0.95, by = 0.001)

for (i in 1:length(Corte_2)) {
  Prediccion <- as.factor(ifelse(prediccion_2 <= Corte_2[i], yes = 1, no = 0))
  
  Exactitud_3[i] <- tryCatch(
    {
      confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), Prediccion)$overall[1]
    },
    error = function(e) {
...
plot(x=Corte_3, y = Exactitud_3, type = "l", main ="Punto de corte óptimo", xlab="Corte", ylab="Exactitud")
```
- ubicando tal punto de corte óptimo para maximizar el accuracy

```{r}
corte_optimo_3 = which(Exactitud_3==max(Exactitud_3))
Corte_3[corte_optimo_3]
```
- punto de corte ideal para maximizar la sensibilidad
...
for (i in 1:length(Corte_2)) {
  Prediccion <- as.factor(ifelse(prediccion_3 <= Corte_2[i], yes = 1, no = 0))
...
plot(x=Corte_3, y = Exactitud_3, type = "l", main ="Punto de corte óptimo para la sensibilidad", xlab="Corte", ylab="Exactitud")
```
- ubicando tal punto de corte óptimo para maximizar la sensibilidad
...
for (i in 1:length(Corte_2)) {
  Prediccion <- as.factor(ifelse(prediccion_3 <= Corte_3[i], yes = 1, no = 0))
...
plot(x=Corte_3, y = Exactitud_3, type = "l", main ="Punto de corte óptimo para la especificidad", xlab="Corte", ylab="Exactitud")
```
- ubicando tal punto de corte óptimo para maximizar el especificidad
...
Prediccion_3_1<-as.factor(ifelse(prediccion_3<=Corte_3[corte_optimo_3],yes = 1, no = 0))
confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), 
                             Prediccion_3_1)
...
Prediccion_3_1<-as.factor(ifelse(prediccion_3<=0.44,yes = 1, no = 0))
confusionMatrix(data = as.factor(Train_copy_1_model$Loan.Status), 
                             Prediccion_3_1)
```
...
vif_modelo <- vif(modelo_multiple_2)
...
##### Validación cruzada para los modelos: modelo no. 1 y modelo no. 2
...
En la validación cruzada por k pliegues evaluada para los 2 modelos disponibles se puede interpretar que el error cuadratico medio (MSE, por sus siglas en inglés) del modelo es menor, lo cuál es la intención, para el modelo multiple no.2, que a su vez resulta ser el que elegimos previamente porque tenía practicamente el mismo accuracy en la matriz de confusión, y además se reducian las variables del modelo. 
...
localTrain <- Train_copy_1_model[indices, ]
localTest <- Train_copy_1_model[-indices, ]

# Obtener las predicciones en el conjunto de prueba
predicciones <- predict(modelo_multiple_1, newdata = localTest, type = "response")

# Calcular el MSPR con un modelo de clasifiacion binaria
MSPR <- mean((localTest$Loan.Status == 1) - predicciones)^2
...
# Obtener las predicciones en el conjunto de prueba
predicciones <- predict(modelo_multiple_2, newdata = localTest, type = "response")
...
Una vez que la validación cruzada ha demostrado que el mejor modelo es el modelo no. 2, se análiza la matriz de confusión, se emplean metodos de remuestro y predicción para eventos especificos.

#### Ajuste por matriz de confusión
...
9. La curva ROC depende de el punto de corte optimo? o es una curva ROC general para el modelo sin importar el punto de corte optimo que se busca con el ciclo que maximiza el accuracy en la matriz de confusión.
